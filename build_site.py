...
<style>
  :root {{
    ...
    --bad: #c62828;
    --na: #888;
    --unreviewed-bg: rgba(255, 240, 154, 0.12);
  }}
  * {{ box-sizing: border-box; }}
  ...
  /* Section row: grid with content + status gutter + N commentary columns */
  .section-row {{
    display: grid;
    grid-template-columns: minmax(0, 1fr) 5px repeat(var(--side-count, 1), var(--side-width, 340px));
    gap: 10px;
    padding: 10px 0;
  ...
  /* Status gutter (column 2) — colored segments aligned with each element. */
  .status-strip-col {{
    position: relative;
    min-width: 5px;
  }}
  ...
  .status-seg.compliant     {{ background: var(--ok); }}
  .status-seg.need-info     {{ background: var(--info); }}
  .status-seg.noncompliant  {{ background: var(--bad); }}
  .status-seg.na            {{ background: var(--na); }}

  /* Unreviewed content tint */
  [data-elt-id][data-review="unreviewed"] {{
    background: var(--unreviewed-bg);
  }}
  [data-elt-id][data-review="unreviewed"]:hover {{
    background: rgba(255, 240, 154, 0.30);
  }}
  ...
  /* Per-comment review status buttons */
  .review-status-row {{
    display: flex;
    flex-wrap: wrap;
    gap: 3px;
  ...
  .review-btn:hover {{ background: var(--accent-bg); }}
  .review-btn.active.compliant    {{ background: var(--ok);   color: white; border-color: var(--ok); }}
  .review-btn.active.need-info    {{ background: var(--info); color: white; border-color: var(--info); }}
  .review-btn.active.noncompliant {{ background: var(--bad);  color: white; border-color: var(--bad); }}
  .review-btn.active.na           {{ background: var(--na);   color: white; border-color: var(--na); }}

  /* Read-only status pill (shown on loaded-set comments) */
  .readonly-status {{
    display: inline-block;
    padding: 1px 8px;
    border-radius: 10px;
  ...
  .readonly-status.compliant    {{ background: var(--ok); }}
  .readonly-status.need-info    {{ background: var(--info); }}
  .readonly-status.noncompliant {{ background: var(--bad); }}
  .readonly-status.na           {{ background: var(--na); }}
  .commentary-empty {{
    color: var(--muted);
    font-size: 11px;
    font-style: italic;
...
function updateCommentStatus(docKey, eltId, commentId, status) {{
  const arr = getComments(docKey, eltId);
  const i = arr.findIndex((c) => c.id === commentId);
  if (i < 0) return;
  arr[i].status = status || undefined;
  arr[i].updatedAt = new Date().toISOString();
  setComments(docKey, eltId, arr);
}}

// Returns the highest-priority status among local comments on this element,
// or null if none have a status. Priority: noncompliant > need-info > compliant > na.
const STATUS_PRIORITY = {{ noncompliant: 4, "need-info": 3, compliant: 2, na: 1 }};
function statusForComment(c) {{ return c && c.status ? c.status : null; }}
function ownElementStatus(docKey, eltId) {{
  let best = null, bestP = 0;
  for (const c of getComments(docKey, eltId)) {{
    const s = statusForComment(c);
    if (s && (STATUS_PRIORITY[s] || 0) > bestP) {{ best = s; bestP = STATUS_PRIORITY[s]; }}
  }}
  return best;
}}
// Resolved status: element's own status, else section header's status, else null.
function resolvedElementStatus(docKey, eltId) {{
  const own = ownElementStatus(docKey, eltId);
  if (own) return own;
  const slash = eltId.indexOf("/");
  if (slash >= 0) {{
    const sec = eltId.substring(0, slash);
    return ownElementStatus(docKey, sec);
  }}
  return null;
}}
...
function renderDocPage(docKey) {{
  ...
  for (const chunk of chunks) {{
    ...
    stripCol.className = "status-strip-col";
    row.appendChild(stripCol);
...
// Paint the status gutter and the unreviewed tint for every commentable element
// in a section. The gutter fills the entire section span with the section's
// own status (if any), then overlays each element's own status on top.
function decorateSection(row) {{
  if (!row || !row.querySelector) return;
  const content = row.querySelector(".section-content");
  const strip = row.querySelector(".status-strip-col");
  if (!content || !strip) return;
  const page = row.closest(".doc-page");
  if (!page) return;
  const docKey = page.dataset.docKey;
  const sectionId = row.dataset.sectionId;
  strip.innerHTML = "";

  if (window.innerWidth > 1100) {{
    const rowRect = row.getBoundingClientRect();
    // 1) section-wide fill from the heading's own status (if any).
    const secStatus = ownElementStatus(docKey, sectionId);
    if (secStatus) {{
      const headingEl = content.querySelector('[data-elt-id="' + cssEscape(sectionId) + '"]');
      // Start at the heading's top (or section top if no heading element / preamble).
      const startTop = headingEl ? (headingEl.getBoundingClientRect().top - rowRect.top) : 0;
      const endTop = row.offsetHeight;
      const seg = document.createElement("div");
      seg.className = "status-seg " + secStatus;
      seg.style.top = startTop + "px";
      seg.style.height = Math.max(0, endTop - startTop) + "px";
      strip.appendChild(seg);
    }}
    // 2) per-element override segments.
    content.querySelectorAll("[data-elt-id]").forEach((el) => {{
      const eid = el.dataset.eltId;
      if (eid === sectionId) return; // section already covered above
      const own = ownElementStatus(docKey, eid);
      if (!own) return;
      const rect = el.getBoundingClientRect();
      const seg = document.createElement("div");
      seg.className = "status-seg " + own;
      seg.style.top = (rect.top - rowRect.top) + "px";
      seg.style.height = rect.height + "px";
      strip.appendChild(seg);
    }});
  }}

  // 3) Unreviewed background tint: anything with no resolved status gets one.
  content.querySelectorAll("[data-elt-id]").forEach((el) => {{
    const eid = el.dataset.eltId;
    if (resolvedElementStatus(docKey, eid)) el.removeAttribute("data-review");
    else el.setAttribute("data-review", "unreviewed");
  }});
}}

function decorateAllSections() {{
  document.querySelectorAll(".section-row").forEach(decorateSection);
}}
...
window.addEventListener("resize", () => {{
  ...
  _resizeTimer = setTimeout(() => {{ alignAllSectionRows(); decorateAllSections(); }}, 80);
}});
...
function renderReviewButtons(currentStatus) {{
  let html = '<div class="review-status-row" role="group" aria-label="Review status">';
  for (const opt of STATUS_OPTIONS) {{
    const active = currentStatus === opt.value;
    html += '<button class="review-btn ' + opt.value + (active ? ' active' : '') +
            '" data-review-value="' + opt.value + '" title="Toggle ' + opt.label + '">' +
...
function renderCommentCard({{ docKey, eltId, comment, readonly }}) {{
  ...
  card.dataset.reviewStatus = comment.status || "";
  ...
  // Body: read-only renders markdown + a status pill; editable shows textarea + preview + review buttons.
  if (readonly) {{
    html += '<div class="rendered">' + renderCommentMarkdown(comment.text) + '</div>';
    if (comment.status) {{
      const opt = STATUS_OPTIONS.find((o) => o.value === comment.status);
      html += '<div class="readonly-status ' + comment.status + '">' + escapeHtml(opt ? opt.label : comment.status) + '</div>';
    }}
  }} else {{
    ...
    html += renderReviewButtons(comment.status);
...
document.addEventListener("mousedown", (e) => {{
  ...
  // Review status buttons — handle on mousedown w/ preventDefault so the
  // textarea keeps focus and the empty-card cleanup doesn't fire (an empty
  // comment with a status set must be kept).
  const reviewBtn = e.target.closest(".review-btn[data-review-value]");
  if (reviewBtn) {{
    e.preventDefault();
    const card = reviewBtn.closest(".comment-card");
    if (!card) return;
    const newValue = reviewBtn.dataset.reviewValue;
    const cur = card.dataset.reviewStatus || "";
    const next = cur === newValue ? "" : newValue;
    const docKey = card.dataset.docKey;
    const eltId = card.dataset.eltId;
    const cid = card.dataset.commentId;
    updateCommentStatus(docKey, eltId, cid, next);
    card.dataset.reviewStatus = next;
    card.querySelectorAll(".review-btn").forEach((b) => {{
      b.classList.toggle("active", b.dataset.reviewValue === next);
    }});
    const row = card.closest(".section-row");
    if (row) decorateSection(row);
...
// Blur on a comment textarea → delete the comment if it has no text AND no
// review status. Keeping status-only comments lets the user just mark a
// section "Compliant" without writing any commentary.
document.addEventListener("focusout", (e) => {{
  ...
  if (!card) return;
  if (card.dataset.reviewStatus) return; // keep status-only comments
  const docKey = card.dataset.docKey;
  const eltId = card.dataset.eltId;
  const cid = card.dataset.commentId;
  deleteComment(docKey, eltId, cid);
