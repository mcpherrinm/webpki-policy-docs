...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  li > .elt-handle, tr > .elt-handle {{ right: -32px; }}
  th .elt-handle, td .elt-handle {{ display: none; }}
  /* For table rows, handle goes to the side of the row */
  tr[data-elt-id] {{ position: relative; }}
...
function annotateElementsWithIds(content, sectionId, docKey) {{
  ...
  while ((node = walker.nextNode())) {{
    ...
    if (tag === "p") {{
      ...
      if (lastTd) lastTd.appendChild(makeEltHandle(docKey, eid, "tr"));
    }}
  }}
}}
...
function onHandleClicked(handle) {{
  ...
  if (!localCol) return;
  // If this elt already has a local comment, scroll to/focus the last one.
  ...
  // Otherwise: create a fresh empty comment, render, focus.
