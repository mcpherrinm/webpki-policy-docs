...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .req-card {{
    ...
    margin-bottom: 10px;
    display: grid;
    grid-template-columns: minmax(0, 1fr) repeat(var(--side-count, 1), var(--side-width, 320px));
    gap: 18px;
  ...
  .set-status-pill.reviewed {{ background: var(--ok); }}
  .set-status-pill.needs-info {{ background: var(--info); }}
  .set-status-pill.noncompliant {{ background: var(--bad); }}
  .set-status-pill.unset {{ background: var(--unset-bg); color: #666; }}
...
<header>
  ...
  <div class="nav-row">
    ...
    <div class="controls">
      ...
      <button id="exportBtn" title="Download Local notes + statuses as JSON">Export</button>
      <button id="importFileBtn" title="Load a notes JSON file as a new side-by-side column">Load file…</button>
      <button id="importUrlBtn" title="Load a notes JSON from a URL as a new side-by-side column">Load URL…</button>
...
// === Loaded notes sets (side-by-side columns) ===
const LOADED_SETS_KEY = "webpki-policy-loaded-sets::v1";
let LOADED_SETS = [];
...
function saveLoadedSetsToStorage() {{
  try {{
    localStorage.setItem(LOADED_SETS_KEY, JSON.stringify(LOADED_SETS));
  }} catch (e) {{ console.error("Could not persist loaded sets:", e); }}
}}
...
function normalizeImportedPayload(obj) {{
  ...
  if (obj && typeof obj === "object" && !Array.isArray(obj)) {{
    ...
    if (!obj.notes && !obj.statuses) notes = obj;
  }}
  const cleanNotes = {{}};
  for (const k of Object.keys(notes)) {{
    const v = notes[k];
    if (typeof v === "string" && v.trim() !== "") cleanNotes[k] = v;
  }}
  const cleanStatuses = {{}};
  for (const k of Object.keys(statuses)) {{
    const v = statuses[k];
    if (typeof v === "string" && v !== "unset" && STATUS_OPTIONS.some((o) => o.value === v)) {{
      cleanStatuses[k] = v;
    }}
  }}
  return {{ notes: cleanNotes, statuses: cleanStatuses }};
}}
...
function addLoadedSet(name, payload, source) {{
  ...
  const id = "set-" + Date.now().toString(36) + "-" + Math.random().toString(36).substring(2, 6);
  LOADED_SETS.push({{ id, name, notes: norm.notes, statuses: norm.statuses, source: source || "" }});
...
function renderLoadedSetsBar() {{
  ...
  let html = '<span class="label">Notes columns:</span>';
  html += '<span class="loaded-set-chip local">Local (editable)</span>';
  for (const s of LOADED_SETS) {{
    const nCount = Object.keys(s.notes).length;
    const sCount = Object.keys(s.statuses).length;
    const title = nCount + " note(s), " + sCount + " status(es)" + (s.source ? " from " + s.source : "");
    html += '<span class="loaded-set-chip" title="' + escapeHtml(title) + '">' +
            escapeHtml(s.name) +
            ' <button class="remove-btn" title="Remove this notes column" data-remove-set="' + escapeHtml(s.id) + '">×</button></span>';
  }}
  bar.innerHTML = html;
}}

function renderLoadedSideColumns(reqDocKey, itemId) {{
  if (LOADED_SETS.length === 0) return "";
  let out = "";
  const composite = reqDocKey + "::" + itemId;
  for (const s of LOADED_SETS) {{
    const note = s.notes[composite] || "";
    const status = s.statuses[composite] || "unset";
    const statusLabel = status === "needs-info" ? "needs info" : status;
    out += '<div class="req-side loaded">' +
      '<div class="notes-label">' +
        '<span class="set-name">' + escapeHtml(s.name) + '</span>' +
        '<span class="set-status-pill ' + status + '">' + escapeHtml(statusLabel) + '</span>' +
      '</div>' +
      '<textarea readonly placeholder="(no notes)">' + escapeHtml(note) + '</textarea>' +
      '<div class="meta"><span style="opacity:0.65">read-only</span><span style="font-family:monospace;opacity:0.65">' + escapeHtml(s.name) + '</span></div>' +
    '</div>';
  }}
  return out;
}}
...
function renderRequirementPage(key) {{
  ...
  for (const it of doc.items) {{
    ...
    card.innerHTML =
      ...
      '</div>' +
      '<div class="req-side">' +
        '<div class="notes-label"><span class="set-name">Local</span></div>' +
      ...
      '</div>' +
      renderLoadedSideColumns(key, it.item_id);
  ...
  for (const section of doc.sections) {{
    ...
    for (const f of section.findings) {{
      ...
      card.innerHTML =
        ...
        '</div>' +
        renderLoadedSideColumns(key, f.id);
...
document.addEventListener("click", (e) => {{
  ...
  // Remove a loaded notes set
  const removeBtn = e.target.closest(".loaded-set-chip .remove-btn[data-remove-set]");
  if (removeBtn) {{
    const id = removeBtn.dataset.removeSet;
    if (confirm("Remove this loaded notes column?")) removeLoadedSet(id);
    return;
  }}
...
document.getElementById("importInput").addEventListener("change", (e) => {{
  ...
  reader.onload = (ev) => {{
    try {{
      ...
      const name = prompt("Name this notes column:", defaultName);
...
document.getElementById("importUrlBtn").addEventListener("click", async () => {{
  const url = prompt("Notes JSON URL (must be CORS-accessible):", "");
  if (!url) return;
  try {{
